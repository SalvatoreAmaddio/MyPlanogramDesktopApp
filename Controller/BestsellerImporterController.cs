...
{
    public class BestsellerImporterController : AbstractImporterController
    {
        public BestsellerImporterController() =>
        ButtonLabel = "IMPORT BEST SELLERS";
        ...
        public async override void OpenFileExplorer()
        {
            ...
            await Task.Run(() =>
            {
                ExcelManager = new BestSellerExcelReader(this, FilePath);
                IsWorking = true;
                ExcelManager.Read();
                IsWorking = false;
    ...
    #region 
    public class BestSellerExcelReader : AbstractExcelManager
    {

        public BestSellerExcelReader(BestsellerImporterController controller, string filepath) : base(filepath, controller)
        {

        }

        public override AbstractChecker Checker { get; set; } = new BestsellerChecker();

        public override void ReadLogic(int currentRow, int currentColumn)
        {
            if (!CellIsNull(currentRow, 1) && !CellIsNull(currentRow, 2))
            {
                var okay = Checker.AddItem(CellValue(currentRow, 1));
                if (okay)
                {
    ...
    public class BestsellerChecker : AbstractChecker
    {
        ...
        $"ITEM: {CurrentItem} is a Best Seller";
    }
    #endregion
}
